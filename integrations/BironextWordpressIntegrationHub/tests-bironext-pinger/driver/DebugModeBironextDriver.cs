...
namespace BiroWoocommerceHubTests {
    public class DebugModeBironextDriver : IBironextDriver
    {

        bool debug;
        IBironextDriver next;
        public DebugModeBironextDriver(bool debug, IBironextDriver next)
        {
            this.debug = debug;
            this.next = next;
        }
        ...
        public Task<bool> Validate(bool exc = false)
        {
            return next.Validate(exc);
        }
    }
}
