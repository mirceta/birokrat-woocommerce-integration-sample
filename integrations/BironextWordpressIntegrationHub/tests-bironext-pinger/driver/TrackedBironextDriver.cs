...
namespace BiroWoocommerceHubTests {
    public class TrackedBironextDriver : IBironextDriver {

        IBironextDriver next;

        List<EventOutcome> killEvents;
        List<EventOutcome> startEvents;

        public TrackedBironextDriver(IBironextDriver next) {
            this.next = next;
            killEvents = new List<EventOutcome>();
            startEvents = new List<EventOutcome>();
        }



        #region [IBironextDriver]
        public void Kill() {
            DateTime start = DateTime.Now;
            try {
                next.Kill();
            } catch (Exception ex) {
                killEvents.Add(new EventOutcome() { start = start, end = null, ex = ex });
            }
        }
        ...
        public void Start(string local_server) {
            DateTime start = DateTime.Now;
            try {
                next.Start(local_server);
            } catch (Exception ex) {
                startEvents.Add(new EventOutcome() { start = start, end = null, ex = ex });
            }
        }

        public async Task<bool> Validate(bool exc = false) {
            return await next.Validate(exc);
        }
        #endregion
    }
}