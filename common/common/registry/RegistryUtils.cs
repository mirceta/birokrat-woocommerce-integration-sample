...
namespace si.birokrat.next.common.registration {
    public static class RegistryUtils {
        ...
        public static bool FixRegistryValue(string registryKey, string key, string value, string valueType = "REG_SZ")
        {
            string current = GetRegistryValue(registryKey, key);

            if (current == null || current != value)
            {
                string command = $"reg add \"{registryKey}\" /v \"{key}\" /t {valueType} /d \"{value}\" /f";
                string result = CommandPrompt.Execute(command);
                return result.Contains("The operation completed successfully.");
            }
