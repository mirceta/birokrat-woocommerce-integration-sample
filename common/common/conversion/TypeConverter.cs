...
namespace si.birokrat.next.common.conversion {
    public static class TypeConverter {
        ...
        public static int StringToInteger(string data) {
            int neki = 0;
            bool success = int.TryParse(data, out neki);
            if (success == false) {
                bool positive = true;
                if (data[0] == '-')
                    positive = false;
                if (data.ToCharArray().Where(x => char.IsLetter(x)).ToList().Count > 0)
                    throw new Exception("The input was not a number");
                else
        ...
        public static string DecimalToString(decimal data, string format = "###,###,##0.00", bool slovene = false, bool writeAllDecimalPlaces = false) {
            string decimalPlaces = new string('0', Math.Max(2, GetDecimalPlaces(data)));
            if (writeAllDecimalPlaces)
        ...
        public static string QuantityToString(decimal data, string format = "###,###,##0.00", bool slovene = false, bool writeAllDecimalPlaces = false)
        {
            if ((decimal)((int)data) == data)
                return IntegerToString((int)data);
            string decimalPlaces = new string('0', Math.Max(2, GetDecimalPlaces(data)));
            if (writeAllDecimalPlaces)
                format = $"###,###,##0.{decimalPlaces}";
            return data.ToString(format, slovene ? new CultureInfo("sl-SI") : new CultureInfo("en-US"));
